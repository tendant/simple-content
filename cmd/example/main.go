...
func main() {
	...
	err = executeContentFlow(context.Background(), svc, storageSvc)
	if err != nil {
...
func initializeS3Backend() (simplecontent.BlobStore, error) {
...
func executeContentFlow(ctx context.Context, svc simplecontent.Service, storageSvc simplecontent.StorageService) error {
	...
	err = storageSvc.UploadObject(ctx, simplecontent.UploadObjectRequest{
		ObjectID: object.ID,
		Reader:   file,
	...
	object_meta, err := storageSvc.UpdateObjectMetaFromStorage(ctx, object.ID)
	if err != nil {
	...
	fileInfo, err := storageSvc.GetObjectMetadata(ctx, object.ID)
	if err != nil {
	...
	downloadURL, err := storageSvc.GetDownloadURL(ctx, object.ID)
	if err != nil {
