...
// LinkContentAsDerived marks existing content as derived from a parent.
// This updates the content's derivation_type and creates the parent relationship.
func (s *service) LinkContentAsDerived(ctx context.Context, req LinkContentAsDerivedRequest) error {
	// Verify content exists
	content, err := s.repository.GetContent(ctx, req.ContentID)
	if err != nil {
		return &ContentError{
			ContentID: req.ContentID,
			Op:        "link_as_derived_get_content",
			Err:       ErrContentNotFound,
		}
	}
	...
	// Check if content is already marked as derived
	if content.DerivationType != "" {
		return &ContentError{
			ContentID: req.ContentID,
			Op:        "link_as_derived",
			Err:       fmt.Errorf("content is already marked as derived (type: %s)", content.DerivationType),
		}
	}
	...
	if derivationType == "" {
		return &ContentError{
			ContentID: req.ContentID,
			Op:        "link_as_derived",
			Err:       fmt.Errorf("derivation_type is required"),
		}
	}

	// Update content's derivation_type field
	content.DerivationType = NormalizeDerivationType(derivationType)
	content.UpdatedAt = time.Now().UTC()

	if err := s.repository.UpdateContent(ctx, content); err != nil {
		return &ContentError{
			ContentID: req.ContentID,
			Op:        "link_as_derived_update",
			Err:       err,
		}
	}

	// Create content_derived relationship
	params := CreateDerivedContentParams{
		ParentID:         req.ParentID,
		DerivedContentID: req.ContentID,
		DerivationType:   content.DerivationType,
		Variant:          req.Variant,
	}

	if _, err := s.repository.CreateDerivedContentRelationship(ctx, params); err != nil {
		return &ContentError{
			ContentID: req.ContentID,
			Op:        "link_as_derived_relationship",
			Err:       err,
		}
	}
